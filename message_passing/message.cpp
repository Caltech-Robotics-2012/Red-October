...
{
  // TODO: Could just use MSG_WAITALL
  size_t bytes_read = 0;
  while (bytes_read < length) {
    ssize_t temp = recv(socket, buffer, length - bytes_read, flags);
    if (temp == 0) { // EOF
...
MessagePasser::MessagePasser(const char *process_name)
{
  ...
  for (int other = 0; other < PROCESS_COUNT; other++) {
    ...
    int port = get_port(process_index, other);

    int sockfd = socket(AF_INET, SOCK_STREAM, 0);
    listener_fd[other] = sockfd;
...
MessagePasser::SendStatus MessagePasser::send_message(int other_process_index, Message message)
{
  ...
  if (client_fd[other_process_index] == -1) {
    ...
    portno = get_port(other_process_index, process_index);
    ...
    client_fd[other_process_index] = sockfd;
  }

  send_message_socket(client_fd[other_process_index], message, 0);
  // TODO: handle failure (try to reconnect, then just return fail)
  return SUCCESS;
  // TODO: Actually report failure
...
MessagePasser::ReadStatus MessagePasser::read_message(Message *message)
{
  MessagePasser::ReadStatus status = NO_MESSAGE;
  // TODO: Lock
...
{
  //int p1 = std::min(process_index1, process_index2);
  //int p2 = std::max(process_index1, process_index2);

  //if (p1 < 0 || p1 >= PROCESS_COUNT || p2 < 0 || p2 >= PROCESS_COUNT || p1 == p2) {
  //  return INVALID_PORT;
  //}

  //return MESSAGE_BASE_PORT + (p1) * (2 * PROCESS_COUNT - p1 - 1) / 2 + (p2 - p1 - 1);
